...
OPENAI_API_KEY = "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"
...
def openai_generate_text_from_file(prompt, file_path, temperature=0.0):
    ext = os.path.splitext(file_path)[1].lower()
    # For both images and text-based files, we convert the file to text.
    if ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.txt', '.pdf', '.docx']:
...
def extract_section(prompt, file_path, temperature=0.0):
    output = openai_generate_text_from_file(prompt, file_path, temperature=temperature)
